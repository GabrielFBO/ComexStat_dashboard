...
Promise.all([
    fetch("/web/data/processed/export_month.json")
        .then(response => response.json()),
    fetch("/web/data/processed/import_month.json")
        .then(response => response.json())
])
    .then(([exportsData, importsData]) => {
        ...
        const countries = [
            ...new Set(
                importsData.map(item => item.country_name)
            )
        ].sort();
        const datalist =
        ...
        function buildMonthChart() {
            ...
            // Um país
                ...
                selectedYears.forEach((year, index) => {
                    const values = months.map(month => {
                        const found = importsData.find(item =>
                            item.year === year &&
                            item.month === month &&
